...
yoke — every solid built from the same `AxialFluxMotor` dimensions the
physics models use.
...
`.volume` against the motor's analytic volume properties (they agree to
<0.1%; VTK boolean operations, the flaky alternative, are never used).
...
- **The cutaway wedge removes stator-side material only** — slicing the
  rotor would cut through a moving part mid-animation.
- **Animations are GIF-only** (GitHub and this site render them inline; MP4
  would add a binary ffmpeg wheel for no documentation benefit).
