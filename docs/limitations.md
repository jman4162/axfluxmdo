...
resistance: no speed-dependent cooling, no radial or axial temperature
...
- Mechanical loss defaults to zero (bearing/windage coefficients are
  parameters of `AnnularModel`), so efficiency is optimistic unless you set
  them.
...
- The 2D FEA validation linearizes the annulus at the mean radius (one pole
  pair, periodic), so radial end effects are outside its scope.
